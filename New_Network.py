...
# ================================================================
# [A2] CROSS-ATTENTION BOTTLENECK — FIXED & STABILIZED
# ================================================================
...
class FreqAwareSelfAttention(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        B, C, H, W = x.shape
        lo = F.adaptive_avg_pool2d(x, (self.attn_size, self.attn_size))
        ...
        s = self.attn_size
        tokens = lo.view(B, C, s * s).permute(0, 2, 1)
        attn_out, _ = self.attn(tokens, tokens, tokens)
        attn_out = attn_out.permute(0, 2, 1).view(B, C, s, s)

        lo_updated = F.interpolate(attn_out, size=(H, W), mode='bilinear', align_corners=False)
        
        merged = lo_updated + hi
        merged = merged.clamp(-20, 20)
        
        out = x + self.proj(self.norm(merged))
        return out.clamp(-50, 50)  # final safety
