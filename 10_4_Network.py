...
class DiffusionUNet(nn.Module):
    ...
    def forward(self, x, t):
        ...
        # 3. Encoder with Skip Collection
        s1 = self.enc1(h, emb)
        h = self.down1(s1)
        ...
        # 5. Decoder with Improved Interpolation
        # We use 'bicubic' for better feature preservation
        h = self.up3(h)
        if s3.shape[-2:] != h.shape[-2:]:
            s3 = F.interpolate(s3, size=h.shape[-2:], mode="bicubic", align_corners=False)
        h = self.dec3(torch.cat([h, s3], dim=1), emb)

        h = self.up2(h)
        if s2.shape[-2:] != h.shape[-2:]:
            s2 = F.interpolate(s2, size=h.shape[-2:], mode="bicubic", align_corners=False)
        h = self.dec2(torch.cat([h, s2], dim=1), emb)

        h = self.up1(h)
        if s1.shape[-2:] != h.shape[-2:]:
            s1 = F.interpolate(s1, size=h.shape[-2:], mode="bicubic", align_corners=False)
