...
def loss_intensity(pred_p, gt_p, weight_map, wet_thresh, k_frac=0.1):
    B, _, H, W = pred_p.shape
    k_global = max(1, int(k_frac * H * W))
    sg_mask_count = int((weight_map > 1.5).float().sum().item())
    k_sg = max(1, min(int(k_frac * 0.2 * H * W), sg_mask_count // 2))
    
    wet_mask = (gt_p > wet_thresh).float()
    
    # Global top error pixels
    error_abs = torch.abs(pred_p.view(B, -1) - (gt_p * wet_mask).view(B, -1))
    topk_idx = error_abs.topk(k_global, dim=1).indices
    loss_global = F.mse_loss(
        pred_p.view(B, -1).gather(1, topk_idx),
        (gt_p * wet_mask).view(B, -1).gather(1, topk_idx)
    )
    
    # Singapore intensity (stronger)
    sg_mask = (weight_map > 1.5).float().expand(B, 1, H, W)
    error_sg = torch.abs(pred_p * sg_mask - gt_p * sg_mask * wet_mask)
    topk_idx_sg = error_sg.view(B, -1).topk(k_sg, dim=1).indices
    loss_sg = F.mse_loss(
        (pred_p * sg_mask).view(B, -1).gather(1, topk_idx_sg),
        (gt_p * sg_mask * wet_mask).view(B, -1).gather(1, topk_idx_sg)
    )
    
    return loss_global + 3.0 * loss_sg   # was 4.0 increase to 6.0


def loss_pcc(pred_p, gt_p, weight_map):
    """
    Strong Singapore-focused Dual-PCC (80% weight on SG region)
    """
    B = pred_p.size(0)
    
    # Global weighted PCC (light)
    ...
    # Singapore-only PCC - STRONGER FOCUS
    w_raw = weight_map.squeeze()
    sg_mask = (w_raw > 1.8)   # broader Singapore region
    n_sg = sg_mask.sum().item()
    
    loss_sg = torch.tensor(0.0, device=pred_p.device)
    if n_sg > 80:
        sg_mask_batched = sg_mask.unsqueeze(0).expand(B, -1, -1)
        p_sg = pred_p.squeeze(1)[sg_mask_batched].reshape(B, -1)
        t_sg = gt_p.squeeze(1)[sg_mask_batched].reshape(B, -1)
    ...
    # 80% weight on Singapore PCC - this should help push SG-PCC
    total_pcc_loss = 0.1 * loss_global + 0.9 * loss_sg
    return torch.clamp(total_pcc_loss, min=0.0, max=2.5)
...
def corrdiff_loss_v7_adaptive(F_x, target_Fx, y_noisy, noise, sigma_r,
    ...
    l_intensity = loss_intensity(pred_p, gt_p, weight_map, wet_thresh_precip)
   
    w = adaptive_weights.get_weights()      # orrect
    total = (w['edm'] * l_edm +
             w['pcc'] * l_pcc +
             w['dry'] * l_dry +
...
# =============================================================================
# =============================================================================
# SMARTER ADAPTIVE LOSS WEIGHTS (Stable for Precip Diffusion)
# =============================================================================
class SmartAdaptiveLossWeights:
    def __init__(self, momentum=0.90, stability_factor=0.6):
        self.momentum = momentum
        self.stability_factor = stability_factor   # higher = more conservative
        
        self.weights = {
            'edm': 1.0,
            'pcc': 0.35,
            'dry': 0.25,
            'intensity': 0.18,      # start lower
        }
    ...
    def update(self, loss_dict):
        ...
        # Normalized contribution with stability
        norm = {}
        for k in self.weights:
            raw_norm = loss_dict.get(k, 1.0) / (self.loss_ma[k] + 1e-8)
            norm[k] = raw_norm ** self.stability_factor   # dampens large variations
        
        total_norm = sum(norm.values()) + 1e-8
        adaptive = {k: (norm[k] / total_norm) * 8.0 for k in self.weights}  # base scale = 8.0
        
        # Update with momentum + hard bounds per term
        for k in self.weights:
            new_w = 0.65 * adaptive[k] + 0.35 * self.weights[k]
            
            # Strong per-term constraints
            if k == 'edm':
                new_w = max(0.8, min(1.8, new_w))
            elif k == 'pcc':
                new_w = max(0.25, min(0.8, new_w))
            elif k == 'dry':
                new_w = max(0.20, min(0.6, new_w))
            elif k == 'intensity':
                new_w = max(0.12, min(0.45, new_w))   # ← tight cap on intensity
            
            self.weights[k] = new_w
            self.weight_history[k].append(new_w)
...
def main():
    ...
    if USE_PAPER_EMBEDDING:
        ...
        embedding_fw = EmbeddingFramework(s_freq=0.25, n_noise_steps=10,
    ...
    # ====================== ADAPTIVE WEIGHTS ======================
    # ====================== SMART ADAPTIVE WEIGHTS ======================
    adaptive_w = SmartAdaptiveLossWeights(momentum=0.90, stability_factor=0.65)
    # ===================================================================
    # ============================================================
    ...
    LOSS_KEYS = ['edm', 'pcc', 'dry', 'intensity']
    ...
    for epoch in range(start_epoch, EPOCHS):
        ...
        if n_batches > 0:
            ...
            epoch_metrics = {
                ...
                'pcc': loss_accum['pcc'],
                'dry': loss_accum['dry'],
                'intensity': loss_accum.get('intensity', 0.0)
            }
        ...
        if epoch % 3 == 0 or epoch < 10:
            ...
            log(f"Ep {epoch+1:5d} | "
                ...
                f"int {loss_accum.get('intensity', 0):.3f}({curr_w.get('intensity', 0):.2f})] | "
