...
CHANGES vs v6 (minimal set):
  A. loss_ssim() → replaced with a stable windowed SSIM that returns [0, 0.3]
     Root cause: old version used global L = g.max()-g.min() which gave
     near-zero C1/C2 on normalised fields → SSIM map all near -1 → clamping
     to [0,1] gave 0 → loss = 1.0 = constant, no gradient.
     Fix: use fixed C1=0.01^2, C2=0.03^2 on unit-normalised fields.

  B. loss_intensity() → add pinball term ON TOP of existing heavy-MSE,
     with a small weight so it nudges rather than disrupts.
     The old loss used topk MSE which is fine for global rain but ignores
     the asymmetric heavy-tail structure. Pinball at q=0.95,0.99 adds
     directional gradient for extremes.

  C. Adaptive weight update: remove the double floor-of-6.0 conflict.
     Keep PCC floor at 6.0 but remove the duplicate block.

  D. corrdiff_loss_v5_adaptive: fix the missing 'tail' key in LOSS_KEYS
     that caused KeyError during accumulation.

  E. LR: resume at 1e-6 (not 2e-6) — more conservative for fine-tuning
     a model already at epoch 1581.

  F. Scheduler: keep CosineAnnealing (not warm-restart) — warm restarts
     caused the spike in v7 by jumping LR up when we need to anneal.
...
LOG_FILE  = "run1.log"
SAVE_DIR  = "/lustre/home/hpc/bipink/VIT_Pune_New/Harsh/SRGAN_pipeline/CorrDiff/run1/"
...
def main():
    ...
    val_end   = int(0.85 * n)
    test_size  = n - train_end - val_end
    indices = list(range(n))  # IMPORTANT: dataset must be time-ordered
    ...
    log(f"Dataset: {n} Train {train_end} Val {val_end-train_end} Test {n-val_end}")
