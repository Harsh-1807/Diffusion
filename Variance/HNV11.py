...
=============================================================================
Changes from v10:
  * FactorizedSpatioTemporalAttn added — replaces BnAttn in UNet bottleneck
    — Spatial branch  : pooled multi-head self-attention over H×W positions
    — Temporal branch : T learned time-tokens → self-attention → gate → broadcast
    — Both branches zero-init → identity at init → safe resume from v10 checkpoints
  * UNet gets `use_spatiotemporal_attn=True` flag (default True)
    — when True  : FactorizedSpatioTemporalAttn at bottleneck + 2 shallowest decoder levels
    — when False : original BnAttn (backward compat)
  * T is now actually used inside UNet.forward (was defined but ignored in v10)
...
# ────────────────────────────────────────────────────────────────────────────
# NEW: Factorized Spatiotemporal Attention  (v11)
# ────────────────────────────────────────────────────────────────────────────

class FactorizedSpatioTemporalAttn(nn.Module):
    """
    Factorized spatial + temporal self-attention for video diffusion downscaling.

    Motivation
    ──────────
    Plain channel-concatenation of T low-resolution frames loses the temporal
    structure after the first convolution.  Factorized spatiotemporal attention
    (as in VDM / Make-A-Video) recovers it:

      Spatial branch  — attend across H×W positions (pooled for efficiency)
      Temporal branch — project global features to T explicit time tokens,
                        self-attend across them, gate-broadcast back

    The two branches are applied sequentially on the same feature map.

    API
    ───
    forward(x, T=None)  →  x   same shape [B, C, H, W]

    T : number of temporal frames (default: self.T set at construction)
        passed explicitly at call-site from T_COND so the block is
        agnostic to how temporal frames are packed.

    Safe resume
    ───────────
    Both s_out and t_out are zero-initialized:
      s_out(·) = 0  →  spatial  residual = 0 at init
      t_gate   = 0  →  tanh(0)  = 0       →  temporal residual = 0 at init
    Loading a v10 checkpoint works out-of-the-box.
    """

    def __init__(self, ch: int, T: int = 5, heads: int = 8, spatial_pool: int = 8):
        super().__init__()
        while ch % heads != 0 and heads > 1:
            heads -= 1
        self.H     = heads          # number of attention heads
        self.T     = T              # default temporal frames
        self.s     = spatial_pool   # pooled spatial resolution
        self.d     = ch // heads    # head dimension
        self.scale = self.d ** -0.5

        # ── Spatial branch ────────────────────────────────────────────────
        self.s_norm = nn.GroupNorm(_g(ch), ch)
        self.s_qkv  = nn.Conv2d(ch, ch * 3, kernel_size=1, bias=False)
        self.s_out  = nn.Conv2d(ch, ch, kernel_size=1)
        nn.init.zeros_(self.s_out.weight)
        nn.init.zeros_(self.s_out.bias)

        # ── Temporal branch ───────────────────────────────────────────────
        # Global pool → T explicit time tokens → self-attention → gate
        self.t_norm = nn.GroupNorm(_g(ch), ch)
        self.t_tok  = nn.Linear(ch, T * ch)          # [B,C] → [B, T*C]
        self.t_qkv  = nn.Linear(ch, ch * 3, bias=False)
        self.t_out  = nn.Linear(ch, ch)
        self.t_gate = nn.Parameter(torch.zeros(1))   # scalar gate, zero init
    ...
    def _spatial(self, x: torch.Tensor) -> torch.Tensor:
        B, C, H, W = x.shape
        s = min(self.s, H, W)
        lo = F.adaptive_avg_pool2d(self.s_norm(x), (s, s))   # [B, C, s, s]
        q, k, v = self.s_qkv(lo).chunk(3, dim=1)             # each [B, C, s, s]

        def _rh(t):
            # [B, C, s, s] → [B, H, s*s, d]
            return t.reshape(B, self.H, self.d, s * s).permute(0, 1, 3, 2)

        q, k, v = _rh(q), _rh(k), _rh(v)
        attn = (q @ k.transpose(-2, -1) * self.scale).softmax(-1)
        out  = (attn @ v).permute(0, 1, 3, 2).reshape(B, C, s, s)   # [B,C,s,s]
        out  = self.s_out(out)
        return x + F.interpolate(out, (H, W), mode='bilinear', align_corners=False)

    def _temporal(self, x: torch.Tensor, T: int) -> torch.Tensor:
        B, C, H, W = x.shape

        # Global average pool → single context vector per sample
        ctx  = F.adaptive_avg_pool2d(self.t_norm(x), 1).view(B, C)  # [B, C]

        # Project to T explicit time tokens
        toks = self.t_tok(ctx).view(B, T, C)                         # [B, T, C]

        # Multi-head self-attention across time tokens
        q, k, v = self.t_qkv(toks).chunk(3, dim=-1)                  # each [B,T,C]

        def _rh(t):
            # [B, T, C] → [B, H, T, d]
            return t.reshape(B, T, self.H, self.d).permute(0, 2, 1, 3)

        q, k, v = _rh(q), _rh(k), _rh(v)
        attn = (q @ k.transpose(-2, -1) * self.scale).softmax(-1)
        out  = (attn @ v).permute(0, 2, 1, 3).reshape(B, T, C)       # [B,T,C]

        # Aggregate T tokens → single correction map, gate-broadcast
        agg  = self.t_out(out).mean(dim=1)                            # [B, C]
        return x + self.t_gate.tanh() * agg.view(B, C, 1, 1)

    def forward(self, x: torch.Tensor, T: int = None) -> torch.Tensor:
        T = T or self.T
        x = self._spatial(x)
        x = self._temporal(x, T)
        return x


# ────────────────────────────────────────────────────────────────────────────
# NEW: Dilated Bottleneck  (from v10, unchanged)
# ────────────────────────────────────────────────────────────────────────────

class DilatedBottleneck(nn.Module):
    """
    Multi-rate dilated convolutional bottleneck (ASPP-style, rates 1-2-4-8).
    Zero-init projection → identity at init → safe resume.
    """
...
# ════════════════════════════════════════════════════════════════════════════
# UNET  (Stage 2) — v11: FactorizedSpatioTemporalAttn at bottleneck + 2 dec levels
# ════════════════════════════════════════════════════════════════════════════
...
class UNet(nn.Module):
    """
    Flow-Matching / CorrDiff-Residual UNet  —  v11

    Bottleneck pipeline:
      m1 → st_attn (FactorizedSpatioTemporalAttn) → fft → dilated_bottleneck → m2

    Decoder spatiotemporal attention:
      Applied at the 2 shallowest decoder levels (highest resolution) when
      use_spatiotemporal_attn=True.  Each is a separate FactorizedSpatioTemporalAttn
      instance at that level's channel width.

    All new blocks are zero-init → safe resume from v10 checkpoints.

    Args
    ────
    use_spatiotemporal_attn : bool  (default True)
        True  → FactorizedSpatioTemporalAttn replaces BnAttn in bottleneck
                 + added at 2 decoder levels
        False → BnAttn only (v10 behaviour)
    use_bottleneck_attention : bool kept for backward compat; ignored when
    ...
    def __init__(self, in_channels, out_channels, base_channels=128,
                 ...
                 temporal_frames=3, use_spatiotemporal_attn=True, **kw):
        super().__init__()
        ...
        self.use_spatiotemporal_attn = use_spatiotemporal_attn
        ...
        if use_spatiotemporal_attn:
            # Factorized spatiotemporal attention  (v11, replaces BnAttn)
            st_heads = max(1, ch // 64)
            self.ma  = FactorizedSpatioTemporalAttn(
                ch=ch, T=temporal_frames, heads=st_heads, spatial_pool=8)
        else:
            # Fallback: original BnAttn  (v10 / backward compat)
        ...
        # ── decoder ──────────────────────────────────────────────────────
        # Track channel widths at each decoder level so we can attach
        # FactorizedSpatioTemporalAttn at the 2 shallowest (largest spatial) levels.
        # "Shallowest" = first 2 entries in reversed(channel_mult) → smallest m values.
        self.dec_st_attn = nn.ModuleList()   # one entry per decoder level (Identity or Attn)

        for i, m in enumerate(reversed(channel_mult)):
            oc = base_channels * m
            ...
            # Attach spatiotemporal attn at the 2 shallowest decoder levels
            if use_spatiotemporal_attn and i < 2:
                st_h = max(1, ch // 64)
                self.dec_st_attn.append(
                    FactorizedSpatioTemporalAttn(
                        ch=ch, T=temporal_frames, heads=st_h, spatial_pool=16))
            else:
                self.dec_st_attn.append(nn.Identity())

        self.nrb = nrb          # stored so forward can slice decoder levels cleanly
        self.out = nn.Sequential(
    ...
    def forward(self, x, t, topo=None, global_features=None,
        ...
        # FactorizedSpatioTemporalAttn or BnAttn — both accept (h) or (h, T)
        if self.use_spatiotemporal_attn:
            h = self.ma(h, T)          # ← T passed explicitly
        else:
            h = self.ma(h)             # BnAttn / Identity
        ...
        # ── decoder — spatiotemporal attn after shallowest 2 levels ──────
        # Layout: per level = 1 up-ResBlock + self.nrb plain ResBlocks
        blocks_per_level = 1 + self.nrb
        up_ptr = 0

        for level_idx, st in enumerate(self.dec_st_attn):
            for _ in range(blocks_per_level):
                h = torch.cat([h, sk.pop()], 1)
                h = self.ups[up_ptr](h, emb, topo_in)
                up_ptr += 1

            # Spatiotemporal attention at the 2 shallowest (largest) levels
            if self.use_spatiotemporal_attn and level_idx < 2:
                h = st(h, T)
            # deeper levels: st is Identity → no-op

        return self.out(h)


# ════════════════════════════════════════════════════════════════════════════
# Flow Matching  (Rectified Flow) — unchanged
# ════════════════════════════════════════════════════════════════════════════
...
# ════════════════════════════════════════════════════════════════════════════
# QDM  (Quantile Delta Mapping) — unchanged
# ════════════════════════════════════════════════════════════════════════════
...
class QDM:
    ...
    def fit(self, mp, op):
        ...
        max_el  = 15_000_000
        if op_flat.numel() > max_el:
            idx     = torch.randperm(op_flat.numel())[:max_el]
            op_flat = op_flat[idx]
            mp_flat = mp_flat[idx]
        self.cm      = torch.quantile(mp_flat, self.q)
        self.co      = torch.quantile(op_flat, self.q)
        self._fitted = True
        i95, i99 = int(.95 * self.n), int(.99 * self.n)
