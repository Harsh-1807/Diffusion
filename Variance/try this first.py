...
────────────────────────────
  * T_COND          : 3  → 5   (more temporal history)
  * LAMBDA_PCC      : 0.1 → 0.5 (stronger spatial correlation signal)
  * ACCUM_STEPS     : 1  → 4   (effective batch ×4 → smoother gradients)
  * LR              : 1e-4 → 2e-4 (sqrt(ACCUM_STEPS) scaling)
  * UNET_D2M_CH     : 1  → 2   (UNet receives d2m AND upsampled var_map)
  * DataLoader      : persistent_workers=True, prefetch_factor=2
  * Data transfers  : non_blocking=True everywhere
  * cudnn.benchmark : True (fastest cuDNN kernels)
  * EMA             : updated every optimizer step (not every 10 gradient steps)
  * Augmentation    : GPU-native flips kept; albumentations path unchanged

var_map conditioning design
────────────────────────────
  Regressor (Stage-1, FROZEN): receives var_map via build_coarse_input()
    — same as before, reg d2m_channels stays = 1.
  UNet (Stage-2): receives torch.cat([d2m, var_map_up], dim=1)
    where var_map_up = F.interpolate(var_map, size=fine_res).
    → UNET_D2M_CH = 2.  No architecture change needed (d2m_channels is
    already a constructor arg of UNet).
"""
...
# ══════════════════════════════════════════════════════════════════════════════
BATCH        = 16
ACCUM_STEPS  = 4            # ← was 1; effective batch = 64 per GPU
LR           = 2e-4         # ← was 1e-4; scaled by sqrt(ACCUM_STEPS)
MIN_LR       = LR * 0.01
EPOCHS       = 1500
PATIENCE     = 200
T_COND       = 5            # ← was 3; more temporal context
PRECIP_CH    = 0
BASE_CH      = 256
...
P_CFG_DROP   = 0.15
LAMBDA_SPEC  = 0.2
LAMBDA_PCC   = 0.5          # ← was 0.1; stronger wPCC signal
WEIGHT_DECAY = 1e-3
GRAD_CLIP    = 1.0
EMA_DECAY    = 0.9995
N_ENS        = 2
REG_IN_CH    = 2            # regressor coarse input channels (precip + var_map)
REG_D2M_CH   = 1            # regressor d2m channels (unchanged)
D2M_CH       = 1            # raw d2m channels in dataset
UNET_D2M_CH  = 2            # ← NEW: UNet gets [d2m, var_map_upsampled]
TOPO_CH      = 3
GLOBAL_DIM   = 2
...
# ══════════════════════════════════════════════════════════════════════════════
# 8.  UNET D2M BUILDER  ──  cat(d2m, var_map_upsampled) → 2-channel
# ══════════════════════════════════════════════════════════════════════════════
def build_unet_d2m(d2m, var_map, fine_size):
    """
    Concatenate raw d2m [B,1,H,W] with var_map upsampled to fine resolution.
    Returns [B, UNET_D2M_CH, Hf, Wf] = [B, 2, Hf, Wf].
    Passes var_map directly into the diffusion UNet so it can anchor
    spatial rain patterns to climatology — regressor already sees it
    via build_coarse_input(), now UNet does too.
    """
    var_up = F.interpolate(var_map, size=fine_size, mode='bilinear', align_corners=False)
    if d2m is None:
        return var_up
    return torch.cat([d2m, var_up], dim=1)
...
def setup():
    ...
    # Fastest cuDNN kernels — critical for fixed-size spatial inputs
    torch.backends.cudnn.benchmark = True
    torch.backends.cuda.matmul.allow_tf32 = True   # H100: TF32 matmul ~2× faster
...
def load_regressor(ckpt_path, dev):
    ...
    reg=CorrDiffRegressor(
        ...
        d2m_channels=ck.get("d2m_channels", REG_D2M_CH),   # regressor: d2m only = 1
...
def ddim_sample(raw_model, mu, tc, tp, gf, d2m_unet, edm_schedule, dev):
    """d2m_unet is the 2-channel [d2m, var_map_up] tensor for the UNet."""
    B = mu.shape[0]
    ...
    for i, sigma_cur in enumerate(sigmas):
        ...
        D_pred = raw_model(c_in * x_in, c_n, topo=tp,
                           global_features=gf, d2m=d2m_unet, T=T_COND)
...
def train():
    ...
    # persistent_workers + prefetch_factor → loader fills pipeline while GPU trains
    _loader_kwargs = dict(
        num_workers=8,
        pin_memory=True,
    ...
    # UNet: d2m_channels=UNET_D2M_CH=2 so it receives [d2m, var_map_up]
    model=UNet(
    ...
    if rank==0:
        ...
        print(f"[OPTIM]  LR={LR}  ACCUM={ACCUM_STEPS}  eff_batch={BATCH*ACCUM_STEPS}  "
              f"LAMBDA_PCC={LAMBDA_PCC}")
        print(f"[PERF]   cudnn.benchmark=True  tf32=True  "
              f"persistent_workers=True  prefetch_factor=2")
        hdr=(f"{'Ep':>5}|{loss_label:>10}|{'L_spec':>8}|{'L_pcc':>8}"
             f"|{'Val':>8}|{'wPCC':>7}|{'CRPS':>8}|{'PSD_r':>7}|{'FSS':>6}|{'LR':>9}")
    ...
    for ep in range(start, EPOCHS):
        ...
        _opt_steps=0   # optimizer step counter this epoch

        for step, b in enumerate(trl, 1):
            try:
                # ── non_blocking transfers → CPU↔GPU overlap ─────────────
                ...
                # ── GPU-native spatial augmentation ──────────────────────
                ...
                if step % ACCUM_STEPS == 0:
                    ...
                    opt.zero_grad(set_to_none=True)
                    ema.update(raw)   # ← update every optimizer step (not every 10)
                    _opt_steps += 1
        ...
        if rank==0:
            ...
            ck_base={
                ...
                "d2m_channels":UNET_D2M_CH,   # save UNet's d2m_ch for inference reload
...
class CorrDiffInference:
    def __init__(self, reg_ckpt, unet_ckpt, qdm_ckpt, device,
        ...
        unet_d2m_ch=ck.get("d2m_channels",UNET_D2M_CH)   # 2: d2m + var_map_up
        self.unet=UNet(
    ...
    def predict(self, coarse, var_map, topo, d2m, doy, hour, tc_frames=None):
        ...
        # regressor: raw d2m only
        mu=self.reg(xi,topo=tp,global_features=gf,d2m=d2m)

        # UNet: d2m + var_map_up
        fine_size=mu.shape[-2:]
        d2m_unet=build_unet_d2m(d2m,var_map,fine_size=fine_size)
