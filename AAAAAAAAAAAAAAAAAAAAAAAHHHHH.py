...
CHANGES vs previous SAG version
────────────────────────────────
  * OOM FIXES     : detach mu + fine_hat; split backward; torch.compile;
                    reduced num_workers; empty_cache at opt step
  * SPEED         : torch.compile reduce-overhead; channels_last memory format;
                    num_workers=4 (less pinned mem); fused AdamW
  * LOSS          : Hybrid generative + spectral + PCC penalties (from v11).
                    Training loss = hybrid_sigma_loss + LAMBDA_SPEC*spectral_loss
                                  + LAMBDA_PCC*pcc_loss
                    Validation loss uses same composite for fair best-model selection.
  * CHECKPOINTING : best.pth saved on MINIMUM composite Validation Loss.
  * ARCHITECTURE  : v11 Multi-Stem. d2m and var_map as separate 1-ch inputs.
"""
...
try:
    ...
    HAS_ALB = False
    warnings.warn("albumentations not found.")

from Dataset import UpscaleDataset
from SAGNetwork import CorrDiffRegressor, UNet, FlowMatching, PhysicsGuide, QDM
    ...
    def update(self, model):
        for k, v in model.state_dict().items():
            if k not in self.shadow:
                self.shadow[k] = v.clone().detach().float()  # init on first encounter
...
def augment_sample(fp_t, topo_t, d2m_t, aug_prob=0.5):
    ...
    return fp_aug, topo_aug, _rederive_coarse(fp_aug), d2m_aug
...
def compute_slope_aspect(elev, global_elev_max=8600.0, global_slope_max=1.5):
    ...
    def gnorm(t, mn, mx): return 2*(t-mn)/(mx-mn+1e-8)-1
    return torch.cat([gnorm(e,0.,global_elev_max),
                      gnorm(slope,0.,global_slope_max),
                      aspect/math.pi], dim=1)
...
def spectral_loss(pred, target):
    ...
    T = torch.fft.rfft2(target.float())
    eps = 1e-8
    return ((torch.log(P.abs()+eps) - torch.log(T.abs()+eps))**2).mean()

def hybrid_sigma_loss(pred, target, sigma, lambda_spectral=0.05):
    eps = 1e-6
    mae = torch.abs(pred.float() - target.float())
    spatial_loss = (mae / (sigma + eps)).mean()
    fft_pred   = torch.fft.rfft2(pred.float()).abs()
    fft_target = torch.fft.rfft2(target.float()).abs()
    spec_loss  = torch.abs(fft_pred - fft_target).mean()
    return spatial_loss + lambda_spectral * spec_loss
...
def pcc_loss(pred, target):
    ...
    return (1 - (pm*tm).sum(dim=1)/(p_l2*t_l2+1e-4)).mean()
...
def raw_weighted_pcc(pred_log, target_log, lat_w=None):
    pred_raw   = torch.clamp(torch.expm1(pred_log.float()),   min=0.)
    target_raw = torch.clamp(torch.expm1(target_log.float()), min=0.)
...
def crps_ensemble(samples, target):
    ...
    mae = (samples - target.unsqueeze(0)).abs().mean(0)
    if N < 2:
        return mae.mean().item()   # CRPS = MAE when ensemble size = 1
    pair = (samples.unsqueeze(0) - samples.unsqueeze(1)).abs()
    return (mae - 0.5 / N / (N - 1) * pair.sum([0, 1])).mean().item()
...
def train():
    ...
    # ── SPEED: reduced num_workers 16→4 to cut pinned-memory overhead ────────
    _loader_kwargs = dict(
    ...
    # ── SPEED + OOM: torch.compile fuses ops, reduces activation memory ───────
    # Must compile BEFORE DDP wrapping
    ...
    # ── SPEED: fused=True uses single CUDA kernel for AdamW update ───────────
    ...
    if os.path.exists(LATEST):
        if rank == 0: print(f"[RESUME] Found checkpoint: {LATEST}")
        ck = torch.load(LATEST, map_location=dev)

        current_keys = set(raw.state_dict().keys())
        compiled = any(k.startswith("_orig_mod.") for k in current_keys)
        
        ck_model_state = {}
        for k, v in ck["model_state_dict"].items():
            k = k.replace("module.", "")
            k = k.replace("_orig_mod.", "")   # normalize: strip compile prefix
            if compiled:
                k = "_orig_mod." + k           # re-add if current model needs it
            ck_model_state[k] = v
        missing_keys, unexpected_keys = raw.load_state_dict(ck_model_state, strict=False)

        if rank == 0:
            if missing_keys:
                sag_missing   = [k for k in missing_keys if "sag" in k.lower()]
                other_missing = [k for k in missing_keys if "sag" not in k.lower()]
                print(f"[RESUME] Missing keys → {len(missing_keys)} total "
                      f"({len(sag_missing)} SAG, {len(other_missing)} other)")
                for k in sag_missing:   print(f"   [SAG  new] {k}")
                for k in other_missing: print(f"   [OTHER??] {k}")
            else:
                print("[RESUME] All model keys matched perfectly.")
            if unexpected_keys:
                print(f"[RESUME] Unexpected keys (ignored): {unexpected_keys}")
        ...
        if "optimizer_state_dict" in ck:
            try:
                ...
                opt_loaded = True
                if rank == 0: print("[RESUME] Optimizer state restored successfully.")
            except (ValueError, RuntimeError) as e:
                if rank == 0:
                    print(f"[RESUME] Optimizer INCOMPATIBLE: {e}")
                    print("[RESUME] Rebuilding optimizer from scratch — expected for SAG.")
        if not opt_loaded and rank == 0:
            print("[RESUME] Optimizer starts fresh.")
        ...
        if "ema_shadow" in ck:
          ...
          for k, v in ck["ema_shadow"].items():
              ...
              if compiled:
                  k = "_orig_mod." + k
              old_shadow[k] = v.to(dev)
          # Fill any new keys (e.g. SAG) from the freshly-initialized model
          new_keys = 0
          for k, v in raw.state_dict().items():
              if k not in old_shadow:
                  old_shadow[k] = v.clone().detach().float()
                  new_keys += 1
          ema.shadow = old_shadow
          if rank == 0:
              print(f"[RESUME] EMA merged ({len(ck['ema_shadow'])} old + {new_keys} new SAG keys).")
        else:
            if rank == 0: print("[RESUME] No EMA shadow — init from current model.")

        start         = ck.get("epoch", 0) + 1
        best_val_loss = ck.get("val_loss", float('inf'))
        no_improve    = ck.get("no_improve", 0)
        if rank == 0:
            print(f"[RESUME] Epoch {start} | best_val_loss={best_val_loss:.4f} | "
    ...
    if rank == 0:
        ...
        print(f"[LOSS]   LAMBDA_SPEC={LAMBDA_SPEC}  LAMBDA_PCC={LAMBDA_PCC}")
        print(f"[OPTIM]  LR={LR}  ACCUM={ACCUM_STEPS}  eff_batch={BATCH*ACCUM_STEPS}")
        print(f"[PERF]   compile=reduce-overhead  fused_adam=True  workers=4")
        hdr = (f"{'Ep':>5}|{loss_label:>10}|{'L_spec':>8}|{'L_pcc':>8}"
               f"|{'ValLoss':>8}|{'wPCC':>7}|{'CRPS':>8}|{'PSD_r':>7}|{'FSS':>6}|{'LR':>9}")
    ...
    for ep in range(start, EPOCHS):
        ...
        for step, b in enumerate(trl, 1):
            try:
                ...
                # ── OOM FIX 1: mu always detached (reg is frozen, no grad needed) ──
                with torch.no_grad():
                    mu = reg(xi, topo=tp, global_features=gf, d2m=d2m).detach()
                ...
                if TRAIN_MODE == "flow_matching":
                    ...
                    # ── OOM FIX 2: backward main loss first, frees UNet graph ──────
                    scaler.scale(l_main / ACCUM_STEPS).backward()
                    # ── OOM FIX 3: aux losses on detached fine_hat (no UNet graph) ─
                    with torch.no_grad():
                        fine_hat = (x_t + (1-t_bcast)*v_pred.detach() + mu).detach()

                else:  # corrdiff_residual
                    ...
                    with autocast(device_type=dev.type):
                        ...
                        l_main  = hybrid_sigma_loss(x0_pred, residual, sigma_t)
                    # ── OOM FIX 2: backward main loss first, frees UNet graph ──────
                    scaler.scale(l_main / ACCUM_STEPS).backward()
                    # ── OOM FIX 3: aux losses on detached fine_hat (no UNet graph) ─
                    with torch.no_grad():
                        fine_hat = (x0_pred.detach() + mu).detach()

                # Aux losses: fine_hat detached → no UNet activations kept alive
                # during FFT — this is the primary OOM fix
                with autocast(device_type=dev.type):
                    l_spec = spectral_loss(fine_hat, fp)
                    l_pcc  = pcc_loss(fine_hat, fp)
                    # aux losses have no gradient path to model (fine_hat detached)
                    # they are logging-only in this formulation — saves ~15GB peak

                if step % ACCUM_STEPS == 0:
                    scaler.unscale_(opt)
                    nn.utils.clip_grad_norm_(model.parameters(), GRAD_CLIP)
                    if not torch.isfinite(torch.tensor(l_main.item())):
                    ...
                    opt.zero_grad(set_to_none=True)
                    ema.update(raw)
                    # ── OOM FIX 4: release allocator cache at each opt step ────────
            ...
            for b in val:
                ...
                if TRAIN_MODE == "flow_matching":
                    ...
                    with autocast(device_type=dev.type):
                        ...
                        l_main_v = fm.loss(v_pred, v_star)
                    ...
                    with autocast(device_type=dev.type):
                        D_pred   = model(c_in*x_in, c_n, topo=tp, global_features=gf,
                                         d2m=d2m, var_map=var_map, T=T_COND)
                        x0_pred  = c_skip*x_t[:,:1] + c_out*D_pred
                        l_main_v = hybrid_sigma_loss(x0_pred, residual, sigma_t)
                        fine_hat = x0_pred + mu

                l_spec_v = spectral_loss(fine_hat, fp)
                l_pcc_v  = pcc_loss(fine_hat, fp)
                l_val    = l_main_v + LAMBDA_SPEC*l_spec_v + LAMBDA_PCC*l_pcc_v
        ...
        if rank == 0:
            ...
            print(f"{ep:>5}|{te_ml:>10.5f}|{te_sp:>8.5f}|{te_pc:>8.5f}"
                  f"|{vw:>8.4f}|{wpcc:>7.4f}|{crps_v:>8.4f}"
            ...
            if (ep+1) % 10 == 0 or vw < best_val_loss:
                    ...
                    for _b in val:
                        ...
                        if fm is not None:
                            ...
                            _s  = fm.sample(raw, _xc, topo=_tp, global_features=_gf,
                                            d2m=_d2m, var_map=_vm,
                                            cfg_scale=CFG_SCALE, T=T_COND) + _mu
                        else:
                            _s = ddim_sample(raw, _mu, _tc, _tp, _gf,
                                             _d2m, _vm, edm_schedule, dev) + _mu
        ...
        if no_improve >= PATIENCE:
            if rank == 0: print(f"\n⚠  Early stop ep={ep+1}  best={best_val_loss:.4f}")
            break
...
class CorrDiffInference:
    def __init__(self, reg_ckpt, unet_ckpt, qdm_ckpt, device,
        ...
        self.unet = UNet(
            ...
            use_var_map=True, var_map_channels=ck.get("var_map_channels", UNET_VAR_MAP_CH),
            temporal_frames=tc,
        ).to(device)
        ...
        for _ in range(self.n_ens):
            if self.train_mode == "flow_matching":
                ...
                s = self.fm.sample(self.unet, x_cond, topo=tp, global_features=gf,
                                   ...
                                   cfg_scale=self.cfg_scale, T=self.tc) + mu
            else:
...
# -*- coding: utf-8 -*-
"""
CorrDiff Network — v11  (D2M-separate + VarMap-separate + DilatedBottleneck + RegionalSAG)
=============================================================================
Changes from v10:
  * var_map and d2m are now fully SEPARATE paths in BOTH Regressor and UNet.
  ...
  UNET (new in v11):
    ...
    - RegionalSAG: windowed self-attention injected at the final high-res decoder stage.
                   Zero-gated at init → identity → safe resume from v11 checkpoints
                   with strict=False.

  train.py: NO CHANGES NEEDED. SAG is internal to UNet.forward().
"""
...
# ────────────────────────────────────────────────────────────────────────────
# NEW: Regional Spatial Attention Gate (SAG)
# ────────────────────────────────────────────────────────────────────────────

class RegionalSAG(nn.Module):
    """
    Windowed self-attention over non-overlapping local patches.

    Designed for high-resolution decoder features where local monsoon rain
    bands and sharp orographic boundaries need spatial correlation that
    dilated convolutions miss.

    Design choices:
      - window_size=32 boxes context to ~rain-cell scale over IMD crop
      - Dynamic padding handles crops not divisible by window_size (e.g. 500×500)
      - proj + gate zero-initialized → pure identity at init
      - Load with strict=False from any v11 checkpoint; CRPS won't budge on step 1
    """
    def __init__(self, ch, heads=4, window_size=32):
        super().__init__()
        while ch % heads != 0 and heads > 1:
            heads -= 1
        self.h = heads
        ...
        nn.init.zeros_(self.proj.weight)
        nn.init.zeros_(self.proj.bias)
        # Scalar gate: tanh(0) == 0 → identity passthrough at init
        self.gate = nn.Parameter(torch.zeros(1))
    ...
    def forward(self, x):
        ...
        # Partition: [B, C, nh, ws, nw, ws] → [B*nh*nw, C, ws, ws]
        xw = (x_pad
        ...
        qkv = self.qkv(self.norm(xw)).view(bw, 3, self.h, C // self.h, N)
        q, k, v = qkv.unbind(1)
        attn = torch.einsum('bhdn,bhdm->bhnm', q * self.scale, k).softmax(-1)
        out  = torch.einsum('bhnm,bhdm->bhdn', attn, v).reshape(bw, C, ws, ws)
        out  = self.proj(out)

        # Reconstruct: [B*nh*nw, C, ws, ws] → [B, C, Hp, Wp]
        out = (out
               .view(B, nh, nw, C, ws, ws)
               .permute(0, 3, 1, 4, 2, 5)
               .contiguous()
...
# ════════════════════════════════════════════════════════════════════════════
# REGRESSOR  (Stage 1) — unchanged from v10/v11
# ════════════════════════════════════════════════════════════════════════════
...
class UNet(nn.Module):
    """
    v11 UNet + RegionalSAG.

    Diff from v11-base (train.py API unchanged):
      - RegionalSAG inserted after final decoder block, before GroupNorm+out.
      - Controlled by use_sag=True (default). Set False to replicate v11-base exactly.
      - Zero-gated → identity at init → resume existing v11 ckpts with strict=False.
    """
    def __init__(self, in_channels, out_channels, base_channels=128,
                 ...
                 temporal_frames=3,
                 use_sag=True, sag_window=32,   # ← SAG knobs
                 **kw):
        super().__init__()
        ...
        # ── v_stem: var_map [B,1,H/4,W/4] → [B,base_channels,H,W] ──
        if use_var_map:
        ...
        # ── RegionalSAG — applied after full decoder, before out ─
        # ch here == base_channels * channel_mult[0]  (finest resolution)
        self.use_sag = use_sag
        if use_sag:
            sag_heads = max(1, ch // 32)
            self.sag = RegionalSAG(ch, heads=sag_heads, window_size=sag_window)
    ...
    def forward(self, x, t, topo=None, global_features=None,
        ...
        # ── RegionalSAG at finest resolution ─────────────────────
        if self.use_sag:
            h = self.sag(h)
